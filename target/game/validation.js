...
const validateInput = (game, update) => {
    if (update.id)
        handleError(badRequest, `It is not allowed to update the id`);
    if (update.board) {
    ...
    if (update.color) {
        if (!validator.isIn(update.color, game_1.colors)) {
            handleError(badRequest, `${capitalizeFirstLetter(update.color)} is not allowed. Make sure that you use follow colors:${game_1.colors.map(color => ` ${color}`)}.`);
        }
    }
};
